...
state_t *emscripten_init() {
  ...
  // makes and saves the asset for the background
  SDL_Rect background_box = {.x = MIN.x, .y = MIN.y, .w = MAX.x, .h = MAX.y};
  asset_t *asset_bgd = asset_make_image(BACKGROUND_PATH, background_box);
  list_add(state->body_assets, asset_bgd);

  // makes and save the asset for the frog
  asset_t *asset_frog = asset_make_image_with_body(
      FROGGER_PATH, *sdl_get_bounding_box(froggy), froggy);
  state->frog = asset_frog;
  ...
  for (size_t r = 3; r < ROWS + 3; r++) {
    ...
    while (cx < MAX.x) {
      ...
      // makes and save the asset for the logs
      asset_t *asset_log = asset_make_image_with_body(
          LOG_PATH, *sdl_get_bounding_box(obstacle), obstacle);
